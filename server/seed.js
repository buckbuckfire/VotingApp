...
const polls = [
    ...
    { question: 'who is the est mutant', options: ['Wolverine', 'Deadpool']},
...
const seed = async () => {
    try {
        ...
        await Promise.all(
            polls.map(async polls => {
                polls.options = poll.options.map(option => ({ option, votes: 0 }));
                const data = await db.Polls.create(polls);
